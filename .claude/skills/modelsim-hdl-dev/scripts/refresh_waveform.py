...
Self-contained waveform refresh using controller infrastructure only.
"""
...
def main():
    """Main entry point for refresh_waveform CLI script."""
    if len(sys.argv) > 1 and sys.argv[1] in ['--help', '-h']:
        print("Usage: refresh_waveform.py")
        print()
        print("Refreshes the waveform display and zooms to full view.")
    ...
    try:
        ...
        result = controller.refresh_waveform()
        ...
        if result['success']:
            print()
            print("✓ SUCCESS: Waveform refreshed and brought to front")
            sys.exit(0)
        else:
            print()
